...
export const BOOK_EQUIPMENT: Record<string, BookEquipmentConfig> = {
  ...
  tw: {
    // "add 20 to the number you have picked" - same as every Grand Master-era/New Order book.
    goldRollBonus: 20,
    // Never actually read for tw — same reasoning as tkt above.
    weaponPool: ALL_WEAPONS,
    healingPotionLabel: 'Potion of Laumspur',
    // "you may keep a maximum of ten articles, including Meals, in your Backpack" - unchanged.
    maxBackpackItems: 10,
    // map.htm's own <h2> drops the "&" ("Gazad Helkona  Surrounding Territories", a double space
    // where the ampersand should be) - a source encoding defect. The <title> tag has it correctly
    // escaped (&amp;), so that's the name used here instead of scraping the buggy heading verbatim.
    applyBaseEquipment: (c) => {
      addSpecialItem(c, { name: 'Map of Gazad Helkona & Surrounding Territories' });
    },
    // Same choose-five, 10-item list as rw (Book 24), but Lute reverts back to Flute (byte-confirmed
    // in equipmnt.htm) - Broadsword (introduced in rw) stays, Quarterstaff does not come back.
    chooseCount: 5,
    chooseOptions: [
      { id: 'axe', label: 'Axe', apply: (c) => addWeaponIfRoom(c, 'Axe') },
      { id: 'sword', label: 'Sword', apply: (c) => addWeaponIfRoom(c, 'Sword') },
    ...
    // Same Kai Weapon Table as vm/tbs/mh/rw (Books 21-24) - confirmed byte-identical (same 10 named
    // weapons, same order) in equipmnt.htm. A carried-over character already has one and skips this
    // step - see the needsKaiWeapon fix in CharacterCreationScreen.tsx.
    kaiWeaponTable: [
      { name: 'Spawnsmite', weaponType: 'Axe' },
  ...
  tfbm: {
    // "add 20 to the number you have picked" - same as every Grand Master-era/New Order book.
    goldRollBonus: 20,
    // Never actually read for tfbm — same reasoning as tkt above.
    weaponPool: ALL_WEAPONS,
    healingPotionLabel: 'Potion of Laumspur',
    // "you may keep a maximum of ten articles, including Meals, in your Backpack" - unchanged.
    maxBackpackItems: 10,
    // equipmnt.htm's own natural-language description ("a map of Bor and its surrounding
    // territories") is used here rather than map.htm's <h2> ("The Kingdom of Bor and Surrounding
    // Territories"), which would read awkwardly as an item name ("Map of The Kingdom of...").
    applyBaseEquipment: (c) => {
      addSpecialItem(c, { name: 'Map of Bor and its Surrounding Territories' });
    },
    // Same choose-five, 10-item list as tw (Book 25), only reordered on the page (Broadsword now
    // listed first) - no item swap this time, confirmed byte-identical categories in equipmnt.htm.
    chooseCount: 5,
    chooseOptions: [
      { id: 'broadsword', label: 'Broadsword', apply: (c) => addWeaponIfRoom(c, 'Broadsword') },
      { id: 'sword', label: 'Sword', apply: (c) => addWeaponIfRoom(c, 'Sword') },
    ...
    // Same Kai Weapon Table as vm/tbs/mh/rw/tw (Books 21-25) - confirmed byte-identical (same 10
    // named weapons, same order) in equipmnt.htm. A carried-over character already has one and skips
    // this step - see the needsKaiWeapon fix in CharacterCreationScreen.tsx.
    kaiWeaponTable: [
      { name: 'Spawnsmite', weaponType: 'Axe' },
      { name: 'Alema', weaponType: 'Axe' },
      { name: 'Magnara', weaponType: 'Axe' },
  ...
  v: {
    // "add 20 to the number you have picked" - same as every Grand Master-era/New Order book.
    goldRollBonus: 20,
    // Never actually read for v — same reasoning as tkt above.
    weaponPool: ALL_WEAPONS,
    healingPotionLabel: 'Potion of Laumspur',
    // "you may keep a maximum of ten articles, including Meals, in your Backpack" - unchanged.
    maxBackpackItems: 10,
    applyBaseEquipment: (c) => {
      addSpecialItem(c, { name: 'Map of Bhanar and the Chai Borderlands' });
    },
    // Same choose-five, 10-item list as tfbm (Book 26), same order this time too - no swap, no
    // reshuffle, confirmed byte-identical in equipmnt.htm.
    chooseCount: 5,
    chooseOptions: [
      { id: 'broadsword', label: 'Broadsword', apply: (c) => addWeaponIfRoom(c, 'Broadsword') },
      { id: 'sword', label: 'Sword', apply: (c) => addWeaponIfRoom(c, 'Sword') },
    ...
    // Same Kai Weapon Table as vm/tbs/mh/rw/tw/tfbm (Books 21-26) - confirmed byte-identical (same 10
    // named weapons, same order) in equipmnt.htm. A carried-over character already has one and skips
    // this step - see the needsKaiWeapon fix in CharacterCreationScreen.tsx.
    kaiWeaponTable: [
      { name: 'Spawnsmite', weaponType: 'Axe' },
  ...
  ths: {
    // "add 20 to the number you have picked" - same as every Grand Master-era/New Order book.
    goldRollBonus: 20,
    // Never actually read for ths — same reasoning as tkt above.
    weaponPool: ALL_WEAPONS,
    healingPotionLabel: 'Potion of Laumspur',
    // "you may keep a maximum of ten articles, including Meals, in your Backpack" - unchanged.
    maxBackpackItems: 10,
    // map.htm's own <h2> drops the "&" ("Chai  The Great Lissan Plain", a double space where the
    // ampersand should be) - same source encoding defect seen in tw's map.htm. The <title> tag has
    // it correctly escaped (&amp;), so that's the name used here instead.
    applyBaseEquipment: (c) => {
      addSpecialItem(c, { name: 'Map of Chai & The Great Lissan Plain' });
    },
    // Same choose-five, 10-item list as v (Book 27), same order - confirmed byte-identical in
    // equipmnt.htm.
    chooseCount: 5,
    chooseOptions: [
      { id: 'broadsword', label: 'Broadsword', apply: (c) => addWeaponIfRoom(c, 'Broadsword') },
    ...
    // Same Kai Weapon Table as vm/tbs/mh/rw/tw/tfbm/v (Books 21-27) - confirmed byte-identical (same
    // 10 named weapons, same order) in equipmnt.htm. A carried-over character already has one and
    // skips this step - see the needsKaiWeapon fix in CharacterCreationScreen.tsx.
    kaiWeaponTable: [
      { name: 'Spawnsmite', weaponType: 'Axe' },
      { name: 'Alema', weaponType: 'Axe' },
      { name: 'Magnara', weaponType: 'Axe' },
  ...
  tsc: {
    // "add 20 to the number you have picked" - same as every Grand Master-era/New Order book.
    goldRollBonus: 20,
    // Never actually read for tsc — same reasoning as tkt above.
    weaponPool: ALL_WEAPONS,
    healingPotionLabel: 'Potion of Laumspur',
    // "you may keep a maximum of ten articles, including Meals, in your Backpack" - unchanged.
    maxBackpackItems: 10,
    applyBaseEquipment: (c) => {
      addSpecialItem(c, { name: 'Map of the Khea-Khanate of Chai' });
    },
    // Same choose-five, 10-item list as ths (Book 28), same order - confirmed byte-identical in
    // equipmnt.htm despite this book's much later (2016) real-world production.
    chooseCount: 5,
    chooseOptions: [
      { id: 'broadsword', label: 'Broadsword', apply: (c) => addWeaponIfRoom(c, 'Broadsword') },
      { id: 'sword', label: 'Sword', apply: (c) => addWeaponIfRoom(c, 'Sword') },
    ...
    // Same Kai Weapon Table as vm/tbs/mh/rw/tw/tfbm/v/ths (Books 21-28) - confirmed byte-identical
    // (same 10 named weapons, same order) in equipmnt.htm. A carried-over character already has one
    // and skips this step - see the needsKaiWeapon fix in CharacterCreationScreen.tsx.
    kaiWeaponTable: [
      { name: 'Spawnsmite', weaponType: 'Axe' },
      { name: 'Alema', weaponType: 'Axe' },
      { name: 'Magnara', weaponType: 'Axe' },
